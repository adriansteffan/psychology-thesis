...
The only modification to this script from the original: changing the expected fps count from 30 to 20.
This (nearly) unchanged script acts as a placeholder while I track down the bug in the refactored version.
...
Original comment at the top of file:
...
class LookingCalibration(object):
    ...
    def calibrate_eyes(self, file, calib_starttime):
        ...
        frameval = math.ceil(fps) // 20  # This is the only line changed from the original to fit our video requirements
